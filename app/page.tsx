...
const focusUnits: Record<LearningFocus, number[]> = {
  general_speaking: [1, 8, 9, 10, 13, 14, 16],
  social: [1, 8, 9, 13, 16],
  cafe: [2, 9, 12, 16],
  work: [6, 10, 14, 15, 16],
  shopping: [3, 10, 12, 16],
  transport: [4, 10, 11, 16],
  home: [5, 10, 13, 14, 16],
  health: [7, 10, 13, 16],
  services: [4, 6, 10, 11, 15, 16],
};
...
const localeCopy: Record<Locale, Record<string, string>> = {
  en: {
    ...
    pricingKicker: 'Choose your access',
    pricingTitle: 'Start free. Unlock the depth when Georgian becomes a habit.',
    pricingBody:
      'The practical phrasebook stays free. The complete dictionary and personalized course are separate upgrades, so you only pay for what you need.',
    freeForever: 'Free forever',
    practicalPhrasebook: 'Practical phrasebook',
    freePlanBody:
      '50 useful phrases for cafés, transport, shopping, and daily life.',
    ...
    faqFreeA:
      'The 50-phrase practical starter is free. The extended dictionary and Guided Learning require their matching access plan.',
    ...
    freeReady: '50 practical phrases free',
    ...
    moreComing: 'Need more than the essentials?',
    moreComingBody:
      'Unlock 9,000 searchable words or build speaking confidence with personalized lessons.',
  ...
  ru: {
    ...
    pricingKicker: 'Выберите доступ',
    pricingTitle:
      'Начните бесплатно. Откройте больше, когда грузинский войдёт в привычку.',
    pricingBody:
      'Практический разговорник остаётся бесплатным. Полный словарь и персональный курс доступны отдельно.',
    freeForever: 'Бесплатно навсегда',
    practicalPhrasebook: 'Практический разговорник',
    freePlanBody:
      '50 полезных фраз для кафе, транспорта, покупок и повседневной жизни.',
    ...
    faqFreeA:
      'Практические 50 фраз бесплатны. Расширенный словарь и учебный курс требуют соответствующего доступа.',
    ...
    freeReady: '50 практических фраз бесплатно',
    ...
    moreComing: 'Нужно больше основных фраз?',
    moreComingBody:
      'Откройте 9 000 слов для поиска или персональный разговорный курс.',
  ...
  ka: {
    ...
    pricingKicker: 'აირჩიეთ წვდომა',
    pricingTitle:
      'დაიწყეთ უფასოდ და გახსენით მეტი, როცა ქართული ჩვევად იქცევა.',
    pricingBody:
      'პრაქტიკული ფრაზები უფასოდ რჩება. სრული ლექსიკონი და პერსონალური კურსი ცალკე წვდომას მოითხოვს.',
    freeForever: 'უფასოდ სამუდამოდ',
    practicalPhrasebook: 'პრაქტიკული ფრაზები',
    freePlanBody:
      '50 სასარგებლო ფრაზა კაფესთვის, ტრანსპორტისთვის, საყიდლებისა და ყოველდღიური ცხოვრებისთვის.',
    ...
    faqFreeA:
      '50 პრაქტიკული ფრაზა უფასოა. გაფართოებულ ლექსიკონსა და სასწავლო კურსს შესაბამისი წვდომა სჭირდება.',
    ...
    freeReady: '50 პრაქტიკული ფრაზა უფასოდ',
    ...
    moreComing: 'საფუძვლებზე მეტი გჭირდებათ?',
    moreComingBody:
      'გახსენით 9 000 საძიებო სიტყვა ან პერსონალური სასაუბრო კურსი.',
  ...
  '@graph': [
    ...
    },
    {
      ...
      offers: [
        ...
        {
          '@type': 'Offer',
          name: 'Guided Learning monthly access',
          price: 19,
          priceCurrency: 'GEL',
        },
      ],
    },
    {
      ...
      mainEntity: [
        ...
        },
        {
          ...
          acceptedAnswer: {
            '@type': 'Answer',
            text: 'No. Guests can use the 50-phrase practical starter. An account and the matching entitlement are required for the extended dictionary or Guided Learning.',
          },
        },
        {
          '@type': 'Question',
...
function AppShell({
  ...
  const [openLearningUnits, setOpenLearningUnits] = useState<number[]>([1]);
  ...
  // Local development keeps the complete course testable without granting a
  // real subscription. Production access still comes only from Supabase.
  const isLocalCoursePreview = process.env.NODE_ENV === 'development';
  ...
  const navigateInApp = (next: Screen) => {
    if (next === 'words' && !canUseDictionary) {
  ...
  useEffect(() => {
    ...
    const dictionaryLocked = !canUseDictionary && screen === 'words';
    if (learningLocked || dictionaryLocked) {
      const redirect = window.setTimeout(() => {
        setUpgradeFocus(dictionaryLocked ? 'phrasebook' : 'guided');
        setScreen('premium');
      }, 0);
      return () => window.clearTimeout(redirect);
    }
  }, [canUseDictionary, canUseLearning, screen]);
  ...
  const dailyPlan = useMemo(() => {
    ...
    const reviewCapacity = Math.min(
      Math.max(1, dailyWordTarget - 1),
      Math.max(2, Math.ceil(dailyWordTarget * 0.5)),
    );
    ...
    return {
      ...
      reviewBacklog: Math.max(0, weak.length + due.length - reviewIds.length),
        ...
        <div className="app-content">
            ...
            <section className="screen words-screen">
              <div className="words-heading">
                <span className="app-eyebrow">
                  {locale === 'ru'
                    ? `Phrasebook Pro · словарь на ${allWords.length.toLocaleString('ru-RU')} слов`
                    : locale === 'ka'
                      ? `Phrasebook Pro · ${allWords.length.toLocaleString('ka-GE')}-სიტყვიანი ლექსიკონი`
                      : `Phrasebook Pro · ${allWords.length.toLocaleString('en-US')}-word dictionary`}
                </span>
                ...
                <>
                  ...
                  <button className="learning-banner" onClick={openLearning}>
                    <span>
                      <small>
                        {locale === 'ru'
                          ? 'Структурированный курс · по подписке'
                          : locale === 'ka'
                            ? 'სტრუქტურირებული კურსი · გამოწერით'
                            : 'Structured course · membership'}
                      </small>
                      <b>
                        {locale === 'ru'
          ...
          {screen === 'category' && (
            <section className="screen category-screen">
              ...
              <div className="locked-card">
                ...
                <Button onClick={openWords}>
                  <LockKeyhole /> Unlock the dictionary
                </Button>
              </div>
            </section>
          )}
          {screen === 'saved' && (
            <section className="screen">
              ...
              {saved.length || (canUseDictionary && savedWords.length) ? (
                <>
                  {saved.length > 0 &&
                    renderPhrases(
            ...
            <section className="screen lesson-path-screen">
              <div className="lesson-path-heading">
                <div>
                  <span className="app-eyebrow">
                    {locale === 'ru'
                      ? 'Guided Learning · подписка'
                      : locale === 'ka'
                        ? 'Guided Learning · გამოწერა'
                        : 'Guided Learning · membership'}
                  </span>
                  <h1>
                    {locale === 'ru'
              ...
              {learnSection === 'today' && (
                <div className="today-learning-panel">
                  ...
                  <div className="today-learning-mix">
                    ...
                    <span>
                      <b>
                        {canUseLearning ? dailyPlan.newCount : dailyWordTarget}
                      </b>
                      {locale === 'ru'
                        ? 'новых'
                        : locale === 'ka'
              ...
              {learnSection === 'today' && (
                <div className="daily-learning-rhythm today-explainer">
                  ...
                  <span>
                    ...
                    <small>
                      {locale === 'ru'
                        ? 'Новые слова возвращаются через 1, 3, 7, 14 и 30 дней. Ошибки — уже через несколько часов.'
                        : locale === 'ka'
                          ? 'ახალი სიტყვები ბრუნდება 1, 3, 7, 14 და 30 დღეში. შეცდომები — რამდენიმე საათში.'
                          : 'New words return after 1, 3, 7, 14, and 30 days. Mistakes return within hours.'}
                    </small>
                  </span>
                </div>
              )}
                ...
                <div className="lesson-units">
                  {speakingUnitNumbers.map((unitNumber) => {
                    ...
                    const unitCompleted = completedSpeakingSteps.filter(
                      (stepNumber) =>
                        unitLessons.some((step) => step.number === stepNumber),
                    ).length;
                    const phaseNumber = Math.ceil(unitNumber / 4);
                    ...
                    return (
                      <section className="lesson-unit" key={unitNumber}>
                        <button
                          ...
                          aria-expanded={openLearningUnits.includes(unitNumber)}
                          onClick={() =>
                            setOpenLearningUnits((current) =>
                              current.includes(unitNumber)
                                ? current.filter((item) => item !== unitNumber)
                                : [...current, unitNumber],
                            )
                          }
                        >
                          <span>
                            {locale === 'ru'
                              ? `Этап ${phaseNumber} · Раздел ${unitNumber}`
                              : locale === 'ka'
                                ? `ეტაპი ${phaseNumber} · ნაწილი ${unitNumber}`
                                : `Stage ${phaseNumber} · Unit ${unitNumber}`}
                          </span>
                        ...
                        {openLearningUnits.includes(unitNumber) && (
                          <div className="lesson-path-list">
                            ...
                            {unitLessons.map((lesson) => {
                              ...
                              const accessLocked = !canUseLearning;
                              const locked =
                                accessLocked ||
                                (!isLocalCoursePreview &&
                                  !complete &&
                  ...
                  <span>
                    {locale === 'ru'
                      ...
                      : locale === 'ka'
                        ...
                        : 'Spiral checks mix older units into every new stage, while daily practice schedules each word before it fades.'}
                  </span>
                </div>
              )}
            </section>
          ...
          {screen === 'premium' && (
            <section className="screen premium-screen">
              <button
                className="back-button"
                onClick={() =>
                  setScreen(upgradeFocus === 'guided' ? 'learn' : 'explore')
                }
              >
                <ArrowLeft />{' '}
                {upgradeFocus === 'guided'
                  ? 'Back to lesson path'
                  : 'Back to free phrases'}
              </button>
              <div className="premium-hero">
                ...
                <p>
                  {upgradeFocus === 'phrasebook'
                    ? 'Unlock the words you need beyond the free 50-phrase starter.'
                    : 'Unlock a daily speaking system shaped around your goals—not another overwhelming word list.'}
                </p>
              </div>
                  ...
                  {upgradeFocus === 'phrasebook' ? (
                    <>
                      ...
                      <div>
                        <Download />
                        <span>
                          <b>Pay once, keep it</b>
                          <small>No recurring fee for dictionary access</small>
                        </span>
                      </div>
                    </>
                  ) : (
                    <>
                      <div>
                        <CalendarDays />
                        <span>
                          <b>Practical A1 survival-speaking path</b>
                          <small>
                            96 guided steps across 16 practical units
                          </small>
                        </span>
                      </div>
                      <div>
                        ...
                        <span>
                          ...
                          <small>
                            253 core words, smart review, and 51 speaking
                            prompts
                          </small>
                        </span>
                      </div>
                ...
                <div className="premium-plan-stack">
                  {upgradeFocus === 'phrasebook' ? (
                    ...
                    <div className="premium-price-card">
                      <span>Personalized membership</span>
                      <h2>
              ...
              <div className="lesson-card">
                ...
                <div className="wave-row">
                  ...
                  <AudioButton
                    ...
                    onPlay={play}
                    text="გამარჯობა"
                    audioUrl="/audio/words/word-035.mp3"
                    large
                  />
                  <i />
                  <i />
                ...
                <div className="settings-grid">
                  ...
                  <article className="settings-card settings-billing">
                    <div className="settings-card-heading">
                      ...
                      <span>
                        <b>Your access</b>
                        <small>Free starter and optional upgrades</small>
                      </span>
                    </div>
                    <div className="plan-status">
                      <span>
                        <b>Phrasebook Pro</b>
                        <small>9,000-word dictionary · ₾60 once</small>
                      </span>
                      <i className={canUseDictionary ? 'active' : ''}>
                        {canUseDictionary ? 'Active' : 'Locked'}
                      </i>
                    </div>
                    <div className="plan-status">
                      <span>
                      ...
                      <i className={canUseLearning ? 'active' : ''}>
                        {canUseLearning ? 'Active' : 'Locked'}
                      </i>
                    </div>
                    <p className="settings-note">
                      Checkout is not connected yet. These access checks are
                      already enforced, so adding the payment provider later
                      will only need to grant the matching entitlement.
                    </p>
                    <Button
                      onClick={() => {
                        setUpgradeFocus(
                          canUseDictionary ? 'guided' : 'phrasebook',
                        );
                        setScreen('premium');
                      }}
                    >
          ...
          <DialogHeader>
            ...
            <DialogDescription>
              {modal === 'install'
                ...
                : 'Checkout is coming next. For now, sign in so your account is ready for the matching access entitlement.'}
            </DialogDescription>
          </DialogHeader>
          {modal === 'install' && (
          ...
          {modal === 'pricing' && (
            <div className="modal-plans">
              <div className="phrasebook-choice">
                <span>
                  <b>Phrasebook Pro</b>
                  <small>9,000-word dictionary · Lifetime access</small>
                </span>
                <strong>₾60</strong>
              </div>
              <div className="recommended">
                <span>
                  <b>Guided Learning</b>
                  <small>16 units · Dictionary included</small>
                </span>
                <strong>₾19/mo</strong>
              </div>
            </div>
