...
const structuredData = {
  ...
  '@graph': [
    { '@type': 'WebSite', name: 'GEO', description: 'Practical Georgian learning for life in Batumi, Tbilisi, and across Georgia.', inLanguage: ['en', 'ru', 'ka'] },
    { '@type': 'SoftwareApplication', name: 'GEO — Learn Georgian', applicationCategory: 'EducationalApplication', operatingSystem: 'Web, Android, iOS, Windows, macOS', description: 'A Georgian phrasebook and five-minute learning app for international residents, Russian speakers, expats, and visitors in Georgia.', inLanguage: ['en', 'ru', 'ka'], areaServed: [{ '@type': 'City', name: 'Batumi' }, { '@type': 'City', name: 'Tbilisi' }, { '@type': 'Country', name: 'Georgia' }], offers: { '@type': 'Offer', price: 0, priceCurrency: 'USD' } },
    { '@type': 'FAQPage', mainEntity: [
      { '@type': 'Question', name: 'Do I need an account to use GEO?', acceptedAnswer: { '@type': 'Answer', text: 'No. You can browse and search Georgian phrases as a guest. An account is needed only to synchronize saved phrases and learning progress.' } },
      { '@type': 'Question', name: 'Can I install GEO as an app?', acceptedAnswer: { '@type': 'Answer', text: 'Yes. GEO is an installable progressive web app on supported browsers and can be added to an iPhone or iPad home screen from Safari.' } },
      { '@type': 'Question', name: 'Is GEO useful for Russian speakers?', acceptedAnswer: { '@type': 'Answer', text: 'Yes. Phrase entries include Russian meanings alongside Georgian, transliteration, and English.' } },
    ] },
  ],
};

function Brand({ onHome }: { onHome: () => void }) {
  return <button className="brand" onClick={onHome} aria-label="Go to the GEO homepage">GEO<span>.</span></button>;
}
...
function Marketing({ openApp, installApp }: { openApp: () => void; installApp: () => void }) {
  ...
  useEffect(() => {
    ...
    void import('motion').then(({ animate, hover, inView, stagger }) => {
      ...
      const stopReveal = inView('.audience-section, .local-section, .demo-pricing, .learning-story, .how-section, .pricing-section, .trust-section, .faq-section, .final-cta', (element) => {
        animate(element, { opacity: [0, 1], y: [38, 0] }, { duration: .75, ease: [.22, 1, .36, 1] });
      }, { amount: .12 });
      const stopHover = hover('.audience-grid article, .local-grid article, .how-grid article, .pricing-grid.redesigned article, .trust-list span, .category-showcase button', (element) => {
      ...
      cleanups.push(() => intro.stop(), () => phones.stop(), () => backPhone.stop(), stopReveal, stopHover);
    });
    let pointerFrame = 0;
    ...
    return () => { disposed = true; cleanups.forEach(cleanup => cleanup()); if (pointerFrame) window.cancelAnimationFrame(pointerFrame); window.removeEventListener('pointermove', trackPointer); };
  }, []);
  ...
  return <main className="marketing">
    ...
    <header className="site-header"><Brand onHome={goHome} /><nav className="desktop-nav" aria-label="Main navigation"><a href="#why">Why GEO</a><a href="#locations">Batumi & Tbilisi</a><a href="#phrases">Phrasebook</a><a href="#pricing">Pricing</a></nav><div className="header-actions"><button className="install-nav" onClick={installApp}><Download /> Install</button><button className="open-app-link" onClick={openApp}>Use on web <ChevronRight /></button><button className="menu-button" onClick={() => setMenuOpen(!menuOpen)} aria-label="Toggle navigation" aria-expanded={menuOpen} aria-controls="mobile-navigation">{menuOpen ? <X /> : <Menu />}</button></div>{menuOpen && <nav id="mobile-navigation" className="mobile-menu" aria-label="Mobile navigation"><a href="#why" onClick={() => setMenuOpen(false)}>Why GEO</a><a href="#locations" onClick={() => setMenuOpen(false)}>Batumi & Tbilisi</a><a href="#phrases" onClick={() => setMenuOpen(false)}>Phrasebook</a><a href="#pricing" onClick={() => setMenuOpen(false)}>Pricing</a><button onClick={installApp}>Install GEO</button><button onClick={openApp}>Use on web</button></nav>}</header>
    <section className="hero"><div className="hero-copy"><span className="eyebrow">Learn Georgian in Batumi & Tbilisi</span><h1>Georgia feels closer<br />when you can <span className="shine-word">speak.</span></h1><p>Practical Georgian for international residents, Russian speakers, expats, and visitors. Find the phrase you need, save it, and build confidence five minutes at a time.</p><div className="hero-buttons"><Button className="primary-cta shiny-button" onClick={openApp}>Use GEO on the web <ChevronRight /></Button><button className="secondary-cta" onClick={installApp}><Download /> Install the app</button></div><div className="hero-proof"><span><CheckCircle2 /> Free browser access</span><span><ShieldCheck /> Secure progress sync</span><span><Globe2 /> Georgian · English · Russian</span></div></div><div className="hero-visual" aria-label="GEO app preview"><img className="skyline" src="/batumi-line.webp" width="1600" height="533" alt="Line illustration of the Batumi waterfront" fetchPriority="high" /><span className="geo-cross" aria-hidden="true">+</span><span className="flag-orbit orbit-one" aria-hidden="true" /><span className="flag-orbit orbit-two" aria-hidden="true" /><div className="phone phone-back"><div className="phone-notch" /><div className="mini-status">9:41 <span>•••</span></div><h4>Real-life situations</h4>{categories.slice(0, 5).map(({ name, icon: Icon, tone }) => <div className="mini-category" key={name}><span className={`mini-icon ${tone}`}><Icon /></span><b>{name}</b><ChevronRight /></div>)}</div><div className="phone phone-front"><div className="phone-notch" /><div className="mini-status">9:41 <span>•••</span></div><span className="mini-back">‹ Essentials</span><span className="mini-pill">Say it now</span><div className="mini-phrase"><strong>გამარჯობა</strong><em>gamarjoba</em><p>Hello · Привет</p><AudioButton id="hero-phone" playing={playing} onPlay={play} large /></div></div></div></section>
    ...
    <section className="local-section" id="locations"><div><span className="section-kicker">Learn where you live</span><h2>Georgian for Batumi, Tbilisi, and everyday life.</h2><p>Use GEO at the café downstairs, in a Bolt or taxi, at the bazaar, with a landlord, or when meeting Georgian friends. English and Russian translations make the first step easier.</p></div><div className="local-grid"><article><span>Batumi</span><h3>Georgian for life by the Black Sea</h3><p>Tourism, cafés, transport, shopping, directions, and neighborly conversation.</p><a href="/learn-georgian-batumi">Learn Georgian in Batumi <ChevronRight /></a></article><article><span>Tbilisi</span><h3>Speak through the capital</h3><p>Metro trips, restaurants, markets, services, workdays, and meeting people.</p><a href="/learn-georgian-tbilisi">Learn Georgian in Tbilisi <ChevronRight /></a></article><article><span>Русский → ქართული</span><h3>Made approachable for Russian speakers</h3><p>Search with Russian meanings while learning Georgian script and transliteration.</p><a href="/learn-georgian-for-russian-speakers">Для русскоязычных <ChevronRight /></a></article></div></section>
    ...
    <section className="faq-section"><span className="section-kicker">Questions, answered</span><h2>Before you begin.</h2><div><details><summary>Do I need an account?</summary><p>No. Browse and search as a guest. Sign in only when you want saved phrases and learning progress on more than one device.</p></details><details><summary>Can I install GEO and use it offline?</summary><p>Yes. GEO is an installable PWA with a cached core interface and offline fallback. Account sync and uncached learning content still require a connection.</p></details><details><summary>Can I buy the paid plans?</summary><p>Not yet. The prices are the intended launch prices. GEO does not currently collect payment details or start trials.</p></details><details><summary>Is every phrase native-reviewed?</summary><p>The production editorial and recording workflow is still being established. The current library is a starter set, not the finished 1,000-phrase catalog.</p></details></div></section>
    <section className="final-cta"><img src="/batumi-line.webp" width="1600" height="533" loading="lazy" alt="" /><span className="section-kicker light">Start where you are</span><h2>Your next Georgian phrase is one tap away.</h2><p>Open GEO, try a phrase, and make today’s conversation a little easier.</p><Button onClick={openApp}>Open GEO free <ChevronRight /></Button></section>
    <PublicFooter />
    <CookieNotice />
    <div className={`sticky-install ${showInstall ? 'visible' : ''}`}><span className="sticky-logo">GEO</span><div><b>Practical Georgian</b><small>Web or installable app</small></div><span className="sticky-actions"><button onClick={installApp}><Download /> Install</button><Button onClick={openApp}>Use web</Button></span></div>
  </main>;
}

function AppShell({ exitToSite, installApp, openModal, openAuth }: { exitToSite: () => void; installApp: () => void; openModal: (kind: 'install' | 'pricing') => void; openAuth: () => void }) {
  ...
  useEffect(() => {
    ...
    void import('@/lib/supabase/client').then(supabaseModule => {
      if (active && supabaseModule.isSupabaseConfigured) setSupabase(supabaseModule.createClient());
    });
    return () => { active = false; };
  }, []);
  const play = (id: string) => { setPlaying(id); window.setTimeout(() => setPlaying(null), 1100); };
...
function AuthDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  const submit = async (event: { preventDefault: () => void }) => {
    ...
    if (!supabaseModule.isSupabaseConfigured) { setStatus('Add the project publishable key to enable account access.'); return; }
    setBusy(true);
...
export default function HomePage() {
  const [mode, setMode] = useState<'marketing' | 'app'>('marketing'); const [modal, setModal] = useState<'install' | 'pricing' | null>(null); const [authOpen, setAuthOpen] = useState(false); const [installPrompt, setInstallPrompt] = useState<BeforeInstallPromptEvent | null>(null);
  useEffect(() => {
    if (window.matchMedia('(display-mode: standalone)').matches || location.hash === '#app') window.setTimeout(() => setMode('app'), 0);
    ...
    const installed = () => { setInstallPrompt(null); setMode('app'); };
  ...
  const installApp = async () => {
    if (window.matchMedia('(display-mode: standalone)').matches) { openApp(); return; }
    if (!installPrompt) { setModal('install'); return; }
    await installPrompt.prompt();
    const choice = await installPrompt.userChoice;
    if (choice.outcome === 'accepted') setInstallPrompt(null);
  };
  const exitToSite = () => { setMode('marketing'); history.replaceState(null, '', location.pathname); window.scrollTo(0,0); };
  return <>{mode === 'marketing' ? <Marketing openApp={openApp} installApp={() => void installApp()} /> : <AppShell exitToSite={exitToSite} installApp={() => void installApp()} openModal={setModal} openAuth={() => setAuthOpen(true)} />}<Dialog open={modal !== null} onOpenChange={(open) => !open && setModal(null)}><DialogContent className="mock-dialog"><DialogHeader><span className="dialog-icon">{modal === 'install' ? <Download /> : <Star />}</span><DialogTitle>{modal === 'install' ? 'Install GEO' : 'Choose how you want to learn'}</DialogTitle><DialogDescription>{modal === 'install' ? 'On iPhone or iPad, tap Share, then “Add to Home Screen.” On Chrome or Edge, use “Install app” in the browser menu if the install prompt is not shown. GEO’s core interface is cached for quick launch and essential offline access.' : 'Checkout is not enabled yet, so no payment will be processed.'}</DialogDescription></DialogHeader>{modal === 'pricing' && <div className="modal-plans"><button><span><b>Full phrasebook</b><small>Lifetime access</small></span><strong>$19.99</strong></button><button className="recommended"><span><b>Guided Learning</b><small>7 days free</small></span><strong>$6.99/mo</strong></button></div>}<div className="dialog-actions">{modal === 'install' && <Button variant="outline" onClick={openApp}>Use on web</Button>}<Button className="dialog-done" onClick={() => setModal(null)}>{modal === 'install' ? 'Close' : 'Continue preview'}</Button></div></DialogContent></Dialog><AuthDialog open={authOpen} onOpenChange={setAuthOpen} /></>;
}
